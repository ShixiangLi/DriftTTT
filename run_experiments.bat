...
set /p "SEEDS=Training seeds, comma-separated [config default]: "
set "SEED_ARG="
if not "%SEEDS%"=="" set "SEED_ARG=--seeds %SEEDS%"

set /p "GPUS=Parallel GPU IDs, comma-separated [serial]: "
if "%GPUS%"=="" (
    "%PYTHON%" -m scripts.run_experiments --dataset "%DATASET%" --subsets "%SUBSETS%" --mixers "%MIXERS%" !SEED_ARG!
) else (
    set /p "JOBS_PER_GPU=Concurrent jobs per GPU [1]: "
    if "!JOBS_PER_GPU!"=="" set "JOBS_PER_GPU=1"
    "%PYTHON%" -m scripts.run_experiments --dataset "%DATASET%" --subsets "%SUBSETS%" --mixers "%MIXERS%" !SEED_ARG! --gpus "%GPUS%" --jobs-per-gpu "!JOBS_PER_GPU!"
)
exit /b %ERRORLEVEL%
